// 32-bit carry-lookahead adder built from four 8-bit CLA stages.
// Produces N (negative), Z (zero), C (carry-out), V (overflow) flags.
module adder(
	a,
	b,
	ci,
	...
	assign C = c_msb;
	assign V = c_msb ^ c_30;  // overflow: carry into MSB differs from carry out of MSB

endmodule

// 8-bit carry-lookahead adder slice
module cla_8bit(
	a,
	b,
	ci,
...
// Carry-lookahead logic unit for 8-bit group
...
// Full adder: generate g=a&b, propagate p=a^b, sum s=p^ci
module full_adder(
	a,
	b,
	ci,
